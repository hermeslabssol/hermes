...
use logios_ledger::Receipt;
use logios_primitives::{ComputeUnits, Hash, Pubkey, Signature, Slot, MAX_COMPUTE_UNITS_PER_SLOT};
use logios_runtime::{AccountsDb, Executor, Transaction};
use thiserror::Error;
...
pub enum ProducerError {
    ...
    TransactionFailed {
        ...
        #[source]
        source: logios_runtime::RuntimeError,
    },
}
...
impl<'a> SlotProducer<'a> {
        ...
        F: FnOnce(&[u8]) -> Signature,
    {
        ...
        let mut blockhash = Hash::hash(format!("logios:slot:{}", slot.get()).as_bytes());
...
mod tests {
    use super::*;
    use logios_runtime::{Account, AccountMeta, Instruction, InvokeContext, Lamports, Program};
    use logios_runtime::RuntimeError;
