//! # logios-consensus
//!
//! Slot production and validator accountability for **Logios**.
//!
//! Logios runs Solana's consensus model — a rotating **leader schedule**, slots
//! as the unit of production, and **Tower-BFT** vote-lockout for finality. On
//! devnet there is a single autonomous leader (the Logios agent), so the
...
//!    decision [`Receipt`](logios_ledger::Receipt).
...
pub use logios_primitives::{Epoch, Lamports, Pubkey, Slot};
