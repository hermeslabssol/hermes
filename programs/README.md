...
| `hermes-faucet` | `AvPqHdw2HCY8RAEHVqpRiE98SaQENAuj6xjnH8nvWbF2` | Testnet **$HERMES** SPL faucet with a per-wallet cooldown. |
...
Mints capped amounts of testnet $HERMES to any caller via a `mint_to` CPI into
...
- `initialize(cooldown_slots)` — bind the faucet to a $HERMES mint and set the
  cooldown.
- `drip(amount)` — mint `amount` $HERMES to the caller; reverts while the
