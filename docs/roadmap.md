...
- [x] `hermes-faucet` Anchor program: devnet $LABS drip, one claim per pubkey
  per epoch.
...
- [ ] $LABS SPL mint on pump.fun (CA TBA — token carries no value).
...
- Defending against a compromised leader signing key. The key is a trusted
  component on devnet.
- An economic model for $LABS. The token confers no rights and no value today.
